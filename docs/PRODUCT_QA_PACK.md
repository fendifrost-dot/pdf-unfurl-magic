...
| --- | --- | --- | --- | --- |
| **Acrobat Pro** | Split, merge, edit text, fill forms, combine client packets | Split, extract, merge, **reorder / delete / extract selected pages** (thumbnail strip + Save As), **page rotate** (`/Rotate` on Save As), **password-protected open** (view-only after unlock), in-place text edit, image studio, **AcroForm fill + flatten**, **Find and permanently redact** (search → erase marks; cover box stays visual-only) — shipped (no XFA / field JS) | — | Preflight, PDF/A, Bates numbering, compare, portfolios, XFA / LiveCycle |
...
**Honest read:** Acrobat workshop must-haves (form fill, page rotate, password-protected open, reorder, delete/extract selected pages) are shipped. Genius Scan still needs a real phone pass. DocuSign is cancellable for *your* outbound documents and not for anything a counterparty controls. Photoshop is the weakest case; the image studio is a PDF repair tool, not an image editor, and pretending otherwise will cost you a build cycle for nothing.
