...
describe("Apply / Enhance exit contracts", () => {
  ...
  it("copies overlay x into member targetX while locating at extract origin", () => {
    const line = {
      id: "row",
      x: 50,
      y: 640,
    ...
    const members = membersForLinePatch(line);
    const desc = members?.find((member) => member.originalText === "Paid To merchant");
    expect(desc?.x).toBe(50);
    expect(desc?.targetX).toBe(80);
