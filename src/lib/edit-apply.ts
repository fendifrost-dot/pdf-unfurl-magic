...
function patchMemberFromRun(
  run: ColumnField["runs"][number],
  text: string,
): TextPatchMember {
  const originX = run.originX ?? run.x;
  const originY = run.originY ?? run.y;
  const moved = Math.abs(run.x - originX) > 0.05 || Math.abs(run.y - originY) > 0.05;
  return {
    x: originX,
    y: originY,
    width: run.width,
