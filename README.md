...
On `/edit` → Marks, **Cover box** only paints over content (still extractable). **Redact (permanent)** removes intersecting text operators and punches simple image pixels in the exported copy; that cannot be undone. **Find and permanently redact** searches the loaded PDF (case-insensitive), lists hits, and after you confirm creates those same `erase` marks — cover boxes are never search results. Gaps (Form XObject CTM, JPEG punch, vectors) are listed in `src/lib/pdf-redact.ts`.
